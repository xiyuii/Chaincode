...
func MysqlInit() (err error) {
	...
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS users (user_id VARCHAR(50) PRIMARY KEY, username VARCHAR(50) UNIQUE NOT NULL, password VARCHAR(50) NOT NULL, realInfo VARCHAR(100), points INT DEFAULT 5)")
	if err != nil {
		panic(err.Error())
	}
	// 重新配置下数据库连接信息
...
// 更新用户的积分
func UpdatePoints(userID string, pointsToAdd int) error {
	sqlStr := "UPDATE users SET points = points + ? WHERE user_id = ?"
	_, err := db.Exec(sqlStr, pointsToAdd, userID)
	if err != nil {
		return err
	}
	return nil
...
// 获取用户的当前积分
func GetUserPoints(userID string) (int, error) {
	sqlStr := "SELECT points FROM users WHERE user_id = ?"
	var points int
	err := db.QueryRow(sqlStr, userID).Scan(&points)
	if err != nil {
		return 0, err
	}
	return points, nil
