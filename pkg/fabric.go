...
// 链码查询
func ChaincodeQuery(fcn string, args ...string) (string, error) {
	contract, conn, gw := GetContract()
	defer conn.Close()
	defer gw.Close()
	evaluateResult, err := contract.EvaluateTransaction(fcn, args...)
	if err != nil {
...
func newSign() identity.Sign {
	models, err := os.ReadDir(keyPath)
	if err != nil {
		panic(fmt.Errorf("failed to read private key directory: %w", err))
	}
	privateKeyPEM, err := os.ReadFile(path.Join(keyPath, models[0].Name()))

	if err != nil {
		panic(fmt.Errorf("failed to read private key model: %w", err))
	}
...
func loadCertificate(modelname string) (*x509.Certificate, error) {
	certificatePEM, err := os.ReadFile(modelname)
	if err != nil {
		return nil, fmt.Errorf("failed to read certificate model: %w", err)
	}
